...
function [BCostW,ICostW,BCostR,ICostR] = IBCostSeven(x,k,N,Lambda,Lr,Lw)
%Initial variables
...
for j = 2:k
    ...
    Bw(j-1) = quadgk(@(t) (Lambda_W.*t-(x(2*k+1)+sum(x(1:j-1)))).*normcdf((Lambda_W.*t-(x(2*k+1)+sum(x(1:j-1))))./sqrt(Lambda_W.*t))+sqrt(Lambda_W.*t).*normpdf(((x(2*k+1)+sum(x(1:j-1)))-Lambda_W.*t)./sqrt(Lambda_W.*t)),Lw(j-1),Lw(j));
end
